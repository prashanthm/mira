...
TRADE_ANALYST_CARD: AgentCard = card_for_domain(
    ...
    synthesis_hint=with_contract(
        ...
        "line with the tape?\n"
        "- ICT STRUCTURE at entry (the `ict` block, computed as-of the entry): "
        "weigh the BACKTEST-VALIDATED flags — `against_draw` (trading against the "
        "nearer opposing level/draw is the money leak: -$357 avg in testing), "
        "`midday_entry` (11:00-14:00 ET level trap: WR 0.17), and "
        "`htf_setup_aligned` (was there a validated hourly setup, on your side?). "
        "Also read the unswept liquidity, order blocks, fresh FVGs, and the hourly "
        "`htf_setup`. Call out plainly if the trade was against the draw or midday.\n"
        "- One concrete LESSON.\n"
