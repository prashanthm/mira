...
`routing.py` defines the :class:`~mira.model.routing.SpanObserver` Protocol and
emits one :class:`~mira.model.routing.CostLatencySpan` per completed model call
(via ``Router.record_call`` and the ADR-010 gateway). ``cost_spans.py`` maps
those observations onto OpenTelemetry spans; this module maps them onto an
in-memory attribution ledger so cost can be sliced by tenant, domain, tool,
model, and provider, and thresholded for the ADR-044 incident workflow.
...
class AttributedSpan:
    """A :class:`CostLatencySpan` enriched with attribution dimensions.

    ``provider``/``model``/``cost``/``latency_ms`` mirror the routing span;
    ...
    def from_span(
        ...
        *,
        ...
        """Wrap a routing span with attribution dimensions."""
        return cls(
...
class LedgerSpanObserver:
    """A :class:`~mira.model.routing.SpanObserver` that records into a ledger.

    Attaches to the ADR-010 gateway / ADR-011 router exactly like
    :class:`~mira.model.cost_spans.OtelSpanObserver`. Attribution dimensions
    ...
    def emit(self, span: CostLatencySpan) -> None:
        """Wrap the routing span with current attribution dims and record it."""
