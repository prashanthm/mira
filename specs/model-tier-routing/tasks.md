...
- [ ] AC-1: heuristic table deterministic; `TierPolicy.resolve` precedence explicit > hint > heuristic > default
  - verify: `pytest tests/test_tiering.py -q`
- [ ] AC-2: tier partition + missing-tier fallback + budget-downgrade-crosses-tiers; gateway fast path unchanged; `chat` routes + emits spans
...
- [ ] AC-1: absent `MODEL_ROUTES` ⇒ gateway identical to today; present ⇒ router + card-derived TierPolicy; malformed ⇒ raises
...
- [ ] AC-1: each trigger reason fires; grounded/in-bounds/high-score pass through untouched
  - verify: `pytest tests/test_quality.py -q`
- [ ] AC-2: exactly one retry with `context={"model_tier": ...}` + isolated thread id; keep-better; budget wins; decision recorded; bridge round-trip stays byte-equal
